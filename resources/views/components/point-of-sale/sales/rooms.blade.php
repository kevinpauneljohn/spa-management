...
<div class="row" id="room-availability-section">
    @for($room = 1; $room <= $rooms; $room++)
        <div class="col-xl-4 col-lg-6 col-sm-6 col-6 room-holder">
            <div class="card collapsed-card @if(collect($takenRoom)->contains($room)) card-secondary @else card-info @endif">
                <div class="card-header">
                    <h3 class="card-title">Room #{{$room}}</h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-sm @if(collect($takenRoom)->contains($room)) btn-secondary @else btn-info @endif" data-card-widget="collapse" title="Collapse">
